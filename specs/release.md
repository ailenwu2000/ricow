...
> 状态: 2026-09-15 建立; 多平台构建工具(dist)尚未接入, 见 §四。
...
- 工具: `dist`(原 cargo-dist), 由它生成并维护 `.github/workflows/release.yml`(不要手改该文件)。
- 接入步骤(待执行, 属发布基建变更):

```bash
cargo install cargo-dist --locked    # 维护者本机执行
dist init                            # 交互式: 选 targets / installers / github attestations
dist plan                            # 本地核对将构建的平台与产物清单
```

- 建议配置: `targets` 见 §五; `installers = ["shell", "powershell"]`; `ci = "github"`; `hosting = "github"`;
  `pr-run-mode = "plan"`; `github-attestations = true`; `install-updater = false`。
- App 名为 `ricow`(CLI 包名即 `ricow`), 产物形如 `ricow-x86_64-unknown-linux-gnu.tar.gz` + `.sha256` + 安装脚本。
