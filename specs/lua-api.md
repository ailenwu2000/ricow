...
- 成交**立即入账后**派发 `on_fill`；返回的订单**立即提交**（回测同 bar 即撮合, 实盘走交易所）；
- 新订单若即时成交, 其 fill 会**再次**触发 `on_fill`（有界递归, 深度上限 8, 超限 WARN 并停止派发）——"成交→决策→挂单"零节流, 不受主时钟/下一根 bar 约束（对齐 NautilusTrader 事件驱动语义）;
- 返回空 `{}` / 不定义返回值 = 无动作（与旧行为一致, 决策等下一次 on_tick）;
- ⚠️ 不要在 `on_fill` 里写"成交即市价反手"的循环逻辑 —— 会在深度上限处被截断并打 WARN。
