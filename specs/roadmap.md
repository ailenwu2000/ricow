...
- **当前基线 (2026-09-26, Linux, 035 实盘静默期兜底心跳落地后实跑)**: `cargo test --workspace` = **579 passed / 0 failed / 22 ignored**; 较 578: +1(`test_live_decision_tick_calls_on_tick_and_submits`)。035 (来源 = 034 §五 #10, 重新验证后唯一"价值高改动小"项): 实盘主循环加固定 30s 心跳(`QUOTE_HEARTBEAT_SECS`), quote 流**未断但静默**时用最后一次 orderbook 调 `on_tick` 兜底决策; Quote 分支的"on_tick 产单 → 提交 → 即成交刷仓"抽取为共用出口 `live_decision_tick` (心跳与 Quote 同一路径, FR-002); 启动后从未收到 quote 时心跳跳过, 不虚构行情事实(不 tick_kline / 不计 ticks)。回测/dry-run 语义不变。三门禁全绿。
- **前基线 (2026-09-26, Linux, 034 事件驱动决策落地后实跑)**: `cargo test --workspace` = **578 passed / 0 failed / 22 ignored**; 较 576: +2(on_fill 订单链同 bar 闭环 / 病态递归深度封顶)。架构改动(034, 只改引擎零改策略): `Strategy::on_fill` 返回 `Vec<OrderRequest>`, 回测单标的/组合/dry-run 三路径统一"下单→撮合→on_fill"有界递归闭环(深度上限 8 + WARN), 实盘 Fill 分支 on_fill 返回订单立即提交(新成交经用户流天然闭环); Lua `on_fill(ctx, fill)` 可返回订单表(与 on_tick 同一套解析); 不从 on_fill 返回订单的策略回测行为逐分不变。竞品架构优势清单与后续优先级见 `specs/changes/034-event-driven-onfill/plan.md` §五。三门禁全绿。
- **前基线 (2026-09-26, Linux, 033 v3 移除积累币模式后实跑)**: `cargo test --workspace` = **576 passed / 0 failed / 22 ignored**; 较 577: −1(`test_uniswap_v2_grid_accumulate_coin_buy_sizing` 随开关删除)。uniswap_v2_grid 保留 v2 参数 `min_spacing_pct`(默认 0.004, 生效间距 = max(atr_mult×ATR, 下限×价格), 负数禁用; 成本门槛严格 `<` 放行 0.4% 默认值); **移除 `accumulate_coin` 开关**: 180d 同窗对照显示 accum 买入后现金钉投入一半、1:1 买量为负 → 买侧枯竭, 6-7 月崩盘段(96→61)80 天零成交(106 笔 vs 基线 1913 笔), 机制缺陷经用户确认直接移除。有效参照 = 关开关基线: 1913 笔/净 +375.95(+3.76%)/总价值 +24.51%/回撤 21.42%, 导出 `/tmp/ricow-bt-univ2-v2/`(v1 与 accum 对照留档 `/tmp/ricow-bt-univ2-accum/`)。账本核对差 0/拒单 0/停摆 0。三门禁全绿。
- **前基线 (2026-09-26, Linux, 033 现货 Uniswap V2 网格落地后实跑)**: `cargo test --workspace` = **575 passed / 0 failed / 22 ignored**; 较 566: +9(033 集成测试: ATR 通道未就绪/缺 start_price FATAL/成本门槛停机/激活半仓建仓与含费 1:1 挂单量/买成交恢复 1:1(同 bar 双成交语义)/卖成交恢复 1:1/无建仓只挂买单/状态快照断点续接字段/账本交叉核对)。新增策略 `uniswap_v2_grid`(现货 Uniswap V2 网格, 033): 真实账本 C+Q 恒 1:1 价值平衡, 平衡价 ± atr_mult×ATR(1h, 14 周期)挂单, 含费修正量精确恢复 1:1, 成交后平衡价 := 成交价并全撤重挂; 修复两处清单/读取缺陷(atr_period 用 num() 读, config_i64 在 CLI 浮点参数下返回 0; 清单 type "int"→"i64"); 建仓成交计入 fill_count(规范 §B)。SOLUSDT 1m×180d 回测(投入 10000, start_price=81.53): 1928 笔成交(买 943/卖 985), 净盈亏 +374.68(+3.75%, 已实现 +421.80 − 费 47.12), 总价值 +24.50% vs 满仓持有 +49.97%(单边上涨中 LP 固有跑输, 如实呈现), 策略最大回撤 21.42% vs 持有 38.62%, 夏普 1.51; 账本交叉核对差 0, 拒单 0, 停摆 0。导出四件套 `/tmp/ricow-bt-univ2/`。三门禁全绿。
- **前基线 (2026-09-26, Linux, 回测规范 v1 + 报告「回测概览」区块落地后实跑)**: `cargo test --workspace` = **566 passed / 0 failed / 22 ignored**; 较 565: +1(概览字段单测 `test_report_overview_fields_v1`)。新增: `specs/backtest.md` §〇 **回测规范 v1**(A1-A7 概览必备/B 交易统计/C 策略适配义务/D 导出四件套/E 验收纪律, 所有回测交付共同遵守); 报告最前新增「回测概览」区块(区间/投入资金/首次成交/标的价格/期末持仓/净盈亏分解/基准对照)与「成交统计」区块(盈利/亏损笔数/最佳最差单笔/总成交额/单笔均名义); 引擎新增展示字段 start_time/end_time/first_open_price/first_entry/final_pos_avg_price/final_pos_dir/close_pnl_realized/winning/losing/best/worst/total_turnover(纯展示, 不改记账); terms 术语表 +9 词条(SC-009 双向核对含概览标签)。SOLUSDT 1m×180d 复跑: 既有指标逐位不变(324 笔/权益 2149.57/+7.48%/夏普 0.84), `close_pnl_realized` == 策略 `stat_close_pnl`(52.132172) 交叉核对一致。三门禁全绿。
  - 同日追加(用户口径, 详见 032 converge §十): **建仓不计算 flag**(建仓 lot 被卖出不 flag+1)与**卖价锚定栈顶成本**(有仓位不随成交上移, 栈空才追踪上移)两处合约版语义修正(与现货显式差异 2/3), 修复"建仓越厚成交越少"缺陷; 多组建仓名义(100/1000/5000 U)180d 对比验证 flag 峰值 +0、成交与建仓脱钩。
- **前基线 (2026-09-26, Linux, 032 v2 合约配对做多重写后实跑)**: `cargo test --workspace` = **565 passed / 0 failed / 22 ignored**; 较 566: −1(空头镜像用例随"砍空头改纯配对做多"设计变更删除)。策略 v2: **`paired_grid_futures_long`**(改名, 为将来做空版 `paired_grid_futures_short` 让位) 重写为只做多配对网格, 行为逻辑与现货 `paired_grid` 逐行一致(5 点合约化差异; 唯一语义差异=现货"无仓无单即结束"不移植, 网格持续运行), 测试改 5 例 + 等价性改公共前缀逐笔一致(现货 finished 后合约版继续成交); 引擎零改动。逐条见 `specs/changes/032-futures-paired-grid/converge.md` §九。
- **前基线 (2026-09-26, Linux, 032 复审停摆根因修复后实跑, 未及记档)**: 566 passed / 0 failed / 22 ignored; 增量 = 限价撮合诚实性(fill=min/max(open,limit)) 2 例 + runner 成交派发时序重排断言 + 清单默认值注入/等价性用例; 修复内容: ①cap_close 1e-9 削裁删除 ②runner 时序(撮合先于决策入账) ③on_fill 逐层扣减 ④equity 补逐仓钱包 ⑤CLOSE- fill 派发策略。逐条见 converge.md §八。

- **前基线 (2026-09-26, Linux, 032+ 回测可观测性增强后实跑)**: `cargo test --workspace` = **559 passed / 0 failed / 22 ignored**; 较上次 554: **+5 通过 / ignored 不变**; 增量 = 回测 fill 时间戳(bar 口径)/ force_close_all 期末强平(双侧平净+标注+分侧盈亏符号)/ 开关关闭回归 / runner close_at_end+strategy_stats 透传 / export_backtest 四文件落盘。新增 CLI `--close-at-end` 与 `--export-dir`; 报告新增分侧盈亏/分侧笔数/峰值名义敞口/策略统计区块; 修复 fill 时间戳墙钟缺陷与 MAX_FILLS 截断。SOLUSDT 1m×14 天真实回测冒烟通过(期末强平干净口径, 导出四文件齐全)。三门禁全绿。逐条见 `specs/changes/032-futures-paired-grid/converge.md` §七。
- **前基线 (2026-09-26, Linux, 032 合约双向配对网格落地后实跑)**: `cargo test --workspace` = **554 passed / 0 failed / 22 ignored**; 较上次 538: **+16 通过 / ignored 不变**; 增量 = 回测 isolated_liq_price/开减仓重算/mark 刷新单测 + lua 绑定 pos_liq/pos_mark 单测 + DryRun hedge(双仓/按侧平仓/reduce_only 封顶/保证金/liq WARN)+ catalog 新可选字段单测 + builtin_tests 合约集成 5 例 + 架构护栏豁免。三门禁全绿(fmt 0 差异 / clippy -D warnings 0 / test 0 failed); 三模式真实冒烟全过(回测真实 K 线 / DryRun 免凭据 / demo-fapi 双向下单+停机平仓干净), 逐条见 `specs/changes/032-futures-paired-grid/converge.md`。附带修复预存在缺陷: Live 同 tick 批量挂单 client_order_id 空串撞号(BN 400), place_order 加毫秒+序号唯一戳。
