...
**版本**: 1.2.0 | **批准日期**: 2026-09-01 | **最后修订**: 2026-09-21

> 修订记录:
> - 1.2.0 (2026-09-21) — **原则一网络口径扩展**(028 D1/FR-038): 平台自身出站仍限交易所 API + 内置数据源 + 用户自配 LLM;
>   新增"用户策略可经 `http:get` 自取任意 URL(平台不限制域名、不背书, 用户自担)", 沙箱无原生网络能力不变。
...
**平台自身**的出站访问仅限: 交易所 API、内置数据源(Nasdaq / Yahoo)、用户自配的 LLM API。
**用户策略**可经平台通道 `http:get`(仅 GET; 墙钟超时 10s、响应体上限 5MB)自取任意 URL ——
平台不限制域名, 也不为这些请求的正确性/合规性背书, 风险由用户自担(028 D1);
Lua 沙箱本身没有 socket / 文件 / 进程能力, 网络只能走这条通道。
