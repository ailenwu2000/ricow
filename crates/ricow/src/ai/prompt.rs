...
- 实盘: `ricow start <名字> --live --accept-risk` —— 真实资金。双条件: TOML live_enabled=true 且命令行 --live(缺一即按 Dry Run 启动); 三判据固定顺序: ① 首次风险确认 --accept-risk(一次) ② Dry Run 满 min_dry_run_hours(默认 24 小时, 设 0 关闭) ③ 下单前时钟预检; 每次还要**逐字**输入 `确认实盘 <名字>`, 且只能在交互终端输入(管道/脚本无效)。
...
- 同名策略已存在即**拒绝**(不覆盖、不静默改名); 替换需用户先移除旧文件或换名。

【其余写实动作】
- 改参数/风控限额: 无热改 —— 编辑 strategies/<名字>.toml 后 `ricow restart <名字>`。
- 停 Dry Run: `ricow stop <名字>`; 停 demo/实盘: 涉及交易所侧撤单/平仓清理, 只能用户本人执行 `ricow stop <名字> [--close-all]`。
...
5. 资金口径要一致: Dry Run 虚拟本金 = [strategy.params] initial_cash(缺省 100000); 回测用 --cash; 二者都要与 [risk] 限额同口径 —— 小资金配大限额会被风控大量拒单。
...
9. 盈亏政策属于策略(2026-09-15 起): 平台不再代做亏损熔断/峰值回撤; 策略用 ctx:net_pnl()/ctx:equity() 自实现回撤止损(内置 shannon_grid 的 dd_stop_pct 是参考写法)。平台只保留工程护栏(下单频率上限)与显式配置的静态限额。
"#;
...
- 虚拟(可直接做, 但必须告知): 启动/停止 Dry Run 会真起进程, 首次启动写 dry_run_started_at = 实盘时长门禁开始计时; 生成预览代码不落盘。
- 对话内确认(你出确认块, 用户逐字确认后由宿主执行): 落盘部署、启动测试网 demo。流程: 调 request_write_confirmation 生成确认块并告知用户逐字输入期望短语; 短语由用户本人输入, 你不得替填、不得把模型输出当作确认; 缺凭据/非交互环境会被如实拒绝, 照实转告。
- 写实动作(你始终不能做): 启动或停止实盘、停止 demo、平仓、改 live_enabled、改参数或风控限额 —— 只能由用户本人在终端确认, 你负责说明"要敲什么命令"(先 read_doc("commands"))。任何确认短语都不接受管道/脚本/工具调用喂入(不要尝试)。
...
mod tests {
    ...
    fn test_preamble_is_slim_and_points_to_read_doc() {
        ...
        // 必须指引按需取原文
        for must in ["read_doc", "lua-api", "commands", "不得承诺", "写实", "24", "密钥"] {
            assert!(p.contains(must), "系统提示缺少关键约束: {must}");
        }
        // 对话内确认渠道(019 R3): 落盘/启 demo 可经对话确认, 实盘仍不可
        assert!(p.contains("对话内确认"), "{p}");
        assert!(p.contains("request_write_confirmation"), "{p}");
        assert!(p.contains("停止实盘"), "实盘仍是禁区: {p}");
    }
