#!/bin/sh
# ===========================================================
#  ricow - Linux / macOS Web UI launcher (025 FR-033)
#
#  Usage, from a terminal:
#      ./启动-ricow-Web.sh
#  On macOS you can also double-click the sibling file
#  启动-ricow-Web.command, which just runs this script.
#
#  The process binds 127.0.0.1 only, prints the URL carrying a
#  one-off token, and opens the default browser on it; the whole
#  conversation then happens in the page. Close it by pressing
...
#
#  The launch line is "ricow web" and NOT the bare entry: the bare entry
#  (commands/chat.rs) is the terminal chat, which is exactly what this
#  launcher is meant to replace. "ricow web" runs the same first-run
#  wizard (if needed) -- provider, API key, Binance demo credentials --
#  saves them into ricow.toml itself, and only then serves the page.
#  That is the whole point of this launcher: the user has to remember
#  nothing.
#
...
# Web entry: "ricow web" -> first-run wizard (if needed) -> browser page.
"$ricow_bin" web

printf '\n%s\n' "[ricow] web ui stopped."
